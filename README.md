<div align="center">

# :zap: A A F T A B • V I J A P U R A :zap:

### Frontend Engineer • React.js Specialist • UI Engineer

<br/>

<!-- <img src="https://capsule-render.vercel.app/api?type=waving&color=0:00F7FF,100:8A2BE2&height=120&section=header"/> -->

</div>

---

# :brain: WHO AM I?


```ts
class Developer {
  name = "Aaftab Vijapura";

  role = "Frontend Engineer";

  specialization = [
    "React.js",
    "Next.js",
    "TypeScript",
    "UI Engineering",
  ];

  strengths = [
    "Scalable Architecture",
    "Reusable Components",
    "Performance Optimization",
    "Modern UI Systems",
  ];

  currentFocus = "Building high-quality digital experiences :rocket:";
}
```

---

# :rocket: CURRENT MISSION

<img align="right" width="300" src="https://media.giphy.com/media/L8K62iTDkzGX6/giphy.gif" />

### :telescope: Building
- Scalable Frontend Applications
- Reusable Component Libraries
- Performance Optimized Interfaces
- Clean UI/UX Systems

### :seedling: Exploring
- Advanced Next.js Patterns
- Server Components
- Frontend Architecture
- AI + Frontend Integrations

### :zap: Engineering Philosophy

> “Great products are built with clean architecture and thoughtful UI.”

<br clear="right"/>

---

# :crossed_swords: TECH STACK

<div align="center">

<img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,redux,tailwind,bootstrap,mui,html,css,firebase,aws,git,github,vercel,netlify,figma,vscode,jest" />

</div>

---

# :hammer_and_wrench: ENGINEERING TOOLBOX

| Category | Technologies |
|---|---|
| :art: Frontend | React.js, Next.js, TypeScript, Tailwind CSS |
| :zap: State Management | Redux Toolkit, React Query, Zustand |
| :jigsaw: UI Libraries | MUI, Chakra UI, Bootstrap |
| :fire: Backend Services | Firebase, REST APIs |
| :rocket: Deployment | Vercel, Netlify, AWS |
| :test_tube: Testing | Jest, React Testing Library |
| :dart: Design Systems | Figma, Storybook |

---

# :milky_way: DIGITAL DNA

```yaml
Name: Aaftab Vijapura
Role: Frontend Engineer

Focus:
  - Scalable Frontend Systems
  - Performance Optimization
  - Reusable UI Components
  - Modern Web Experiences

Tech Philosophy:
  "Build interfaces users remember."
```

---

# :globe_with_meridians: CONNECT WITH ME

<div align="center">

<a href="mailto:aaftabvijapura@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-111111?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/aaftab-vijapura/">
  <img src="https://img.shields.io/badge/LinkedIn-111111?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/aaftab-vijapura">
  <img src="https://img.shields.io/badge/GitHub-111111?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</div>

---

# :eyes: PROFILE VISITORS

<div align="center">

<img src="https://komarev.com/ghpvc/?username=aaftab-vijapura&label=Visitors&color=6C63FF&style=flat-square"/>

</div>

---

<div align="center">

### :zap: "Clean Code. Thoughtful UI. Better Experiences." :zap:

<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:8A2BE2,100:00F7FF&height=120&section=footer"/>

</div>
